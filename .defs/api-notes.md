...
- **Current name:** `NodeFieldSpec` (in `core/fields.py`).
...
- **Blast radius:** `core/fields.py`, `assembly/builders.py`,
  `definitions/nodes.py`, `__init__.py` `__all__`, tests, and the doc sites
  `README.md` and `.defs/vocab.html` (Exported Names table). The class name is
  never serialized (fixtures hold only field data, and no JSON-schema files
  exist in the repo), so there is no hash impact (field name of the type, not
  serialized data).

## 2. `NodeConfig` vs. term "Node (Config)" — note, do not rename
...
## 3. `node_type` field — note, currently intentional
...
- **Proposed:** if a typed reference is ever modeled, rename to `node_definition_ref`.
  ...
  action.
- **Blast radius:** if ever renamed — `configuration/nodes.py`,
  `assembly/builders.py`, and the serialized field name (which would change
  every graph hash), plus `definitions/nodes.py`,
  `definitions/materialization.py`, and `assembly/subgraphs.py`; the two node
  classes must rename in lockstep. Because it is identity-bearing, a rename
  here is a hash-breaking change and must be treated as a schema-version bump,
  never a silent rename.

## 4. `TerminalError` vs. `NodeError`

- **Current name:** `TerminalError` (`results/graph_runs.py`), a near-duplicate of `NodeOutcome` — it is
  exactly `NodeOutcome` minus the `output` field, restricted to the ERROR/BLOCKED states (not a
  near-duplicate of `NodeError`, which is a nested field of it). The graph-run
  result builder derives `TerminalError` mechanically from a `NodeOutcome`,
  which strengthens the proposal below.
...
- **Blast radius:** `results/graph_runs.py`, `__init__.py` `__all__`, tests,
  and any caller matching on the type. No hash impact.

## 5. `ClassifiedFailure` — Protocol under-described by its name

- **Current name:** `ClassifiedFailure` (`results/failure_diagnostics.py`), a structural `Protocol` a `NodeError`
...
- **Blast radius:** `results/failure_diagnostics.py`, `__init__.py` `__all__`,
  tests, and type annotations at call sites. No hash impact.

## 6. `graph_config_identity_payload` / `graph_config_identity_document` — verify they earn exports

- **Current names:** `graph_config_identity_payload`, `graph_config_identity_document`
  (`identity/graph_config.py`), the public Identity Payload and Identity Document builders.
...
## 7. `GraphValidationError` inside the `GraphExecutionError` taxonomy

- **Current name:** `GraphValidationError` subclasses both `GraphExecutionError` and
  `ValueError` (`core/errors.py`).
...
- **Blast radius:** `core/errors.py`, `__init__.py` `__all__`, tests asserting on the hierarchy,
...
## 8. `GraphDefinition.schema_version` name collision — note, do not rename

- **Current name:** `GraphDefinition.schema_version` (`definitions/graphs.py`).
